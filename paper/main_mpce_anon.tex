...
\noindent\textbf{Keywords:} LLM agents; power grid; runtime verification; interval analysis; power flow; safety.\par
...
\caption{Danger rate among released actions as a function of the ground-truth
sample size $M$ (nominal, Monte Carlo, and interval verifiers). The interval
rate is identically zero; the nominal estimate stabilizes from $M=100$
onward.}
\label{fig:convergence}
\end{figure}

Table~\ref{tab:main} reports the main comparison over 11{,}101(state, action, load-level) cases. Nominal verification, the deterministic-twin
